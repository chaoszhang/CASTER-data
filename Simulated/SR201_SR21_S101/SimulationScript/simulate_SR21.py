...
for t in tree_list:
    ...
    t = t.extract_tree_with_taxa_labels(["T" + str(i * 10) for i in range(21)])
    samplingTime = {}
    samples = []
    print("REP: " + str(rep))
    ...
    for node in t.leaf_nodes():
        ...
        node.edge_length += st
        samples.append(msprime.SampleSet(1, ploidy=20, time=st, population=node.taxon.label))
    ...
    for MUT_RATE_LEVEL in [0.1, 1, 10]:
        print("mutation")
