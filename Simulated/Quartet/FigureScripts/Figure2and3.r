...
pa = ggplot(aes(y=rf,x=time*core/3600,color=method,shape=method),data=d[d$data == "M201" & d$method != "wASTRAL*" & d$condition == "Default",])+
...
ggplot(aes(y=rf,x=time*core/3600,color=method,shape=method),data=d[d$data == "M201" & d$method != "wASTRAL*",])+
  facet_grid(.~condition)+
  geom_point(alpha=0.3)+
  geom_point(data=s[s$data == "M201" & s$method != "wASTRAL*",], size=5)+
  #facet_wrap(V3~.,scales="free")+
  theme_classic()+
  scale_color_manual(values = mycolor, name="")+
  scale_shape_manual(values = myshape, name="")+
  ...
  theme(legend.position=c(0.15,0.75),panel.border = element_rect(fill=NA))+
  theme(plot.tag.position = c(0, 1),plot.tag = element_text(size=14,face = "bold"),
        legend.margin = margin(0,0,0,0,"pt"),legend.box.margin = margin(0,0,0,0,"pt"))

pb = ggplot(aes(y=rf,x=condition,fill=method),data=d[d$data == "M201" & d$method != "wASTRAL*",])+
  ...
  scale_y_continuous(name="Species tree error (FN)", breaks=seq(0, 21, 2)) + 
  theme(legend.position=c(0.12,0.75),panel.border = element_rect(fill=NA))+
  theme(plot.tag.position = c(0, 1),plot.tag = element_text(size=14,face = "bold"),
        legend.margin = margin(0,0,0,0,"pt"),legend.box.margin = margin(0,0,0,0,"pt"))+labs(tag="B"); pb

spb = ggplot(aes(y=time*core/3600,x=condition,fill=method),data=d[d$data == "M201" & d$method != "wASTRAL*",])+
  stat_summary(geom="bar",position = position_dodge(width = 0.8),width=0.8,color="black")+
  stat_summary(geom="errorbar",position = position_dodge(width = 0.8),color="black",width=0.4)+
  theme_classic()+
  scale_fill_manual(values = mycolor, name="")+
...
pd = ggplot(aes(y=rf,x=as.factor(ind),fill=method),data=d[d$data == "M21" & !d$method %in% c("wASTRAL*", "BPP", "snapper") & d$pop == 1e6,])+
  stat_summary(geom="bar",position = position_dodge(width = 0.8),width=0.8,color="black")+
  stat_summary(geom="errorbar",position = position_dodge(width = 0.8),color="black",width=0.4)+
  theme_classic()+
  scale_fill_manual(values = mycolor, name="")+
...
spc = ggplot(aes(y=time*core/3600,x=as.factor(ind),color=method,shape=method,group=method),data=d[d$data == "M21" & !d$method %in% c("wASTRAL*", "BPP", "snapper") & d$pop == 1e6,])+
  ...
  theme(legend.position="right",panel.border = element_rect(fill=NA))+
  theme(plot.tag.position = c(0, 1),plot.tag = element_text(size=14,face = "bold"),
        legend.margin = margin(0,0,0,0,"pt"),legend.box.margin = margin(0,0,0,0,"pt"))+labs(tag="C"); spc
...
t.test(rf ~ method, data = d[d$condition == "Default" & d$method %in% c("CASTER-site","CASTER-pair"),], paired = TRUE, alternative = "less")
t.test(rf ~ method, data = d[d$condition == "Default" & d$method %in% c("RAxML-ng","CASTER-site"),], paired = TRUE, alternative = "less")
t.test(rf ~ method, data = d[d$condition == "10X population size" & d$method %in% c("CASTER-site","CASTER-pair"),], paired = TRUE, alternative = "less")
...
spa = ggplot(aes(rf,color=method,shape=method),data=d[d$data == "M201" & d$method != "wASTRAL*" & d$ploidy == 1 & d$mut == 5e-8 & d$pop == 1e5,])+
  stat_ecdf()+theme_classic()+
  scale_color_manual(values = mycolor, name="")+
  scale_shape_manual(values = myshape, name="")+
  scale_x_continuous(name="Species tree error (FN)", breaks=seq(0, 16, 2))+
  scale_y_continuous(name="ECDF") +
  theme(legend.position=c(0.7,0.3),panel.border = element_rect(fill=NA))+
  theme(plot.tag.position = c(0, 1),plot.tag = element_text(size=14,face = "bold"),
...
pf = ggplot(aes(V1, y = after_stat(y)*20,color=V3),data=d3[d3$V2 == "Correct" & d3$V3 != "wASTRAL",])+
  stat_ecdf()+theme_classic()+
  scale_shape_manual(values = myshape, name="")+
  geom_line(aes(V1, y = 1 - after_stat(y)), data=d3[d3$V2 == "Incorrect" & d3$V3 != "wASTRAL",], stat='ecdf', linetype = "dashed") +
  scale_color_manual(values = mycolor, name="")+
...
pg = ggplot(aes(m = V1, d = (V2 == "Correct"), color = V3, shape = V3), data = d3) + 
  ...
  scale_color_manual(values = c("CASTER-site" = "#66A61E", "CASTER-pair" = "#1B9E77", "wASTRAL" = "#D95F02"), name="")+
  scale_shape_manual(values = c("CASTER-site" = 17, "CASTER-pair" = 19, "wASTRAL" = 88), name="")+
  ...
  theme_classic()+theme(legend.position=c(0.75,0.5)) +
  theme(plot.tag.position = c(0, 1), plot.tag = element_text(size=14,face = "bold"), 
...
ggplot(aes(y=rf,x=method,color=method,shape=method),data=d4)+
  facet_grid(.~len, labeller = label_bquote(cols = "Gene length: " * .(len)))+
  geom_beeswarm(alpha=0.3, cex = 2)+
  geom_beeswarm(data=s4, size=5)+
  #geom_point(data=s[s$data == "M201" & s$method != "wASTRAL*",], size=5)+
  #facet_wrap(V3~.,scales="free")+
  theme_classic()+
  scale_color_manual(values = mycolor, name="")+
  scale_shape_manual(values = myshape, name="")+
  scale_x_discrete(name="Reconstruction method")+
  scale_y_continuous(name="Species tree error (FN)", breaks=seq(0, 21, 5)) + 
  coord_cartesian(ylim = c(0, 10)) +
  theme(legend.position="right", panel.border = element_rect(fill=NA),
        axis.text.x=element_blank(), axis.ticks.x=element_blank()); ggsave("S101.pdf", width = 10, height = 3)
...
ggplot(aes(y=rf,x=method,color=method,shape=method),data=d4)+
  ...
  #coord_cartesian(ylim = c(0, 10)) +
  theme(legend.position="right", panel.border = element_rect(fill=NA),
        axis.text.x=element_blank(), axis.ticks.x=element_blank()); ggsave("S101-supplement.pdf", width = 10, height = 3)
...
dds$method = factor(dds$method, c("CASTER-site", "CASTER-pair", "RAxML-ng", "wASTRAL", "ASTRAL"))

pc = ggplot(aes(y=rf, fill=method, x=as.factor(gene), group=method), data=dds)+
  ...
  theme_classic()+
  scale_fill_manual(values = c("CASTER-site" = "#66A61E", "CASTER-pair" = "#1B9E77", "RAxML-ng" = "#7570B3",
                                "wASTRAL" = "#D95F02", "ASTRAL" = "#E6AB02"), name="")+
  scale_x_discrete(name="Subsampled data", labels = c("2.5%", "5%" ,"10%", "20%", "100%"))+
  scale_y_continuous(name="Species tree error (FN)", breaks=seq(0, 21, 2)) +
  theme(plot.tag.position = c(0, 1),plot.tag = element_text(size=14,face = "bold"),
        legend.margin = margin(0,0,0,0,"pt"),legend.box.margin = margin(0,0,0,0,"pt"),
        legend.position=c(0.8, 0.8))+labs(tag="C"); pc
...
g = arrangeGrob(spa,spb,spc, widths = c(2,3), layout_matrix = rbind(c(1, 3), c(2, 2)))
ggsave(file="Simulation_sup.pdf", g, width = 7.5, height = 6)
...
dq1 = dq[dq$rf == 1 & dq$case == "hard" & dq$len >= 1 & dq$method %in% c("CASTER-site", "CASTER-pair", "RAxML-ng"),]
partS1 = ggplot(aes(x=method, fill=method, alpha=top), data=dq1)+theme_classic()+
  ...
  scale_fill_manual(values=c("#1f78b4","#33a02c","#e31a1c"), name = "Method")+
...
part1 = ggplot(aes(x=method, fill=method, alpha=top), data=dq1[dq1$len < 20 & dq1$mut == "1X",])+theme_classic()+
...
part2 = ggplot(aes(x=as.factor(1000*len),y=rf, color=method,group=method,linetype=top), data=dq2)+theme_classic()+
  ...
  scale_color_manual(values=c("#E6AB02","#1f78b4","#33a02c"), name = "")+
...
partS3 = ggplot(aes(x=method, fill=method, alpha=top), data=dq2[dq2$rf == 1,])+theme_classic()+
  ...
  scale_x_discrete(name="Sequence length (Mbps)")+
  scale_y_continuous(labels = function(x) percent(x/300),name="Species tree error")+
  scale_fill_manual(values=c("#E6AB02","#1f78b4","#33a02c"), name = "Method")+
  scale_alpha_manual(values=c(0.4,0.7,1), name = "Species tree")+
  theme(axis.text.x = element_blank(), axis.ticks.x = element_blank(),
        strip.background.x = element_blank(),
        panel.spacing.x = unit(15,"pt"))+
