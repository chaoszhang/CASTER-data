...
for mu in [1]:
    for mbp in [1]:
        with open("data/quartet/easy/" + "ABCD" + "_" + str(mbp) + ".bpp.ctl", "w") as f:
            f.write(generate(topID = "ABCD", topology = "(((A,B),C),D);", mbp = mbp, mu = mu * 0.005))
        with open("data/quartet/easy/" + "ACBD" + "_" + str(mbp) + ".bpp.ctl", "w") as f:
            f.write(generate(topID = "ACBD", topology = "(((A,C),B),D);", mbp = mbp, mu = mu * 0.005))
        with open("data/quartet/easy/" + "BCAD" + "_" + str(mbp) + ".bpp.ctl", "w") as f:
